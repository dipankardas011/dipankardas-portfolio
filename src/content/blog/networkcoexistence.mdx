...
cover: "/img/blogs/networkcoexistence.webp"
...
<Callout type="important" title="Docker silently drops VM traffic">
  Docker sets the default policy of the `FORWARD` chain to `DROP` on startup. This is evaluated before libvirt's rules, silently dropping all VM traffic to external networks — even when DNS resolution works fine.
</Callout>
...
<Callout type="caution">
  Using `--permanent` without `--reload` means rules are saved but **not yet active**. Always follow up with `sudo firewall-cmd --reload` or your changes won't take effect until the next reboot.
</Callout>
...
<Callout type="note">
  If you restart Docker after applying these rules, Docker may re-insert its `DROP` policy and override your setup. If VMs lose connectivity again after a Docker restart, re-run `sudo firewall-cmd --reload` to restore priority.
</Callout>
