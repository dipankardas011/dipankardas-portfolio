...
cover: "/img/blogs/otel-cover.webp"
...
2.  **Correlated Traces and Logs**: One of the most powerful features is correlating traces with logs. With a single `trace_id`, you can instantly find all the logs associated with a specific request, even as it hops across multiple services. ![searching-traceId](/img/blogs/tracetolog001.webp)
...
<Callout type="note">
  The core pipelining happens under `service.pipelines.(traces|logs|metrics)`. Each pipeline declares its own receivers, processors, and exporters — so traces, logs, and metrics can flow to completely different backends independently.
</Callout>
...
<Callout type="warning">
  Spans are not free. Creating too many adds performance overhead and makes traces noisy and hard to read. Only instrument what has meaningful duration or crosses a system boundary.
</Callout>
...
![Example of correlated traces and logs in Grafana](/img/blogs/tracetolog002.webp)
...
<Callout type="caution">
  Dynamic span names like `GET /users/123` cause **cardinality explosion** in your observability backend — it creates a unique metric series for every user ID. Always use low-cardinality names like `GET /users/{id}` and store the actual ID as a span attribute.
</Callout>
...
![Example of using events within a span](/img/blogs/tracetolog003.webp)
...
<Callout type="tip">
  Start with head-based sampling at 10–20% for production traffic. Switch to tail-based only once you have a clear need — it requires the collector to buffer full traces in memory before making a decision, which increases collector resource usage significantly.
</Callout>
...
![Example of linking spans for a grouped job](/img/blogs/tracetolog005.webp)
