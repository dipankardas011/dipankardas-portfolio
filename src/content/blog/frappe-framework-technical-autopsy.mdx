...
import SpecialThanks from '../../components/SpecialThanks.astro';
...
### Middleware vs. Internal Code Paths

There's a subtle but critical detail most people miss: **permission evaluation happens in the middleware layer**, not for backend's internal code paths. When a request comes in through the API, middleware checks permissions before reaching the controller. That's the surface-level defense.

But internal code paths — `doctype.validate()`, whitelisted server methods, background job handlers — bypass that middleware entirely. The permission checks inside those functions are manual, opt-in, and inconsistently applied. Every write operation that calls `validate()` or a whitelisted method needs its own permission evaluation. If a developer forgets, there's no safety net. The middleware that protects the API surface does nothing for the backend logic that actually modifies data.

This creates a false sense of security. The API looks properly guarded from the outside. Inside, the code runs with whatever permissions the calling context happens to have — which is often Administrator because so much internal code assumes it.

### There Is No Middleware Layer

Here's something most people don't realize: **Frappe doesn't have a middleware layer.**
...
When these gthread workers process requests and eventually idle down (before/after load testing, or post-peak), CPU returns to normal — but memory doesn't. It doesn't release. The frozen GC plus thread-based workers create a one-way memory leak: memory goes up, never comes back down.
...
There's a workaround — though we're not sure about it, and it's not advised for production unless you're confident about the trade-offs:

```shell
--max-requests 1000 --max-requests-jitter 200
```

Lower the `--max-requests` value so workers restart more frequently, reclaiming memory before it gets out of hand. But this is a band-aid. It doesn't fix the underlying issue: `gc.freeze()` on non-forking workers is a design mismatch, and the memory leak is structural.
...
<SpecialThanks
  people={[
    { name: "Lord Zero", url: "https://developing.tools" }
  ]}
/>
