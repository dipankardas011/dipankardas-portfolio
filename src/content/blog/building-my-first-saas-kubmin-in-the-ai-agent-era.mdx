...
And Here is the bonus tip for you I have already made a [Agent SKILL.md](https://dipankar-das.com/skills) for helping you from zero -> production ready authzed setup.
...
For Kubmin-style data, SQL is the natural default: users, orgs, ... Those things benefit from schema, constraints, transactions, and predictable queries.
...
AI agents can write database code quickly. But they cannot decide your consistency model and access patterns for you. Thats exactly where your systems engineer part comes in.
...
For Kubmin, a small k3s setup made sense. A single self-hosted node on Hetzner with enough CPU/RAM buffer gave me a consistent deployment environment: deployments, services, secrets, config, health checks, volumes, jobs, cron jobs, and a path to add nodes later without switch later.
...
Your SaaS should be able to run locally without needing the full production circus.
...
If you understand the feature superficially like its a start small steps towards the goal and the path is unknown, I do not ask an agent to build it. That is how you get architecture by hallucination.
...
On long-term memory, I can comment a bit more now: it is useful, but not because the agent remembers everything.
...
Finally, I keep coming back to this quote. It may become true in terms of writing code, but code is not the whole thing. The system is what matters.
