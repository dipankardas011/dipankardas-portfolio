...
The Copilot **Free** tier caps at 2,000 code completions per month — roughly 66 per day, assuming you code every day. Go over that and you hit a wall until next month. The **Pro** tier ($10/month) moved to usage-based billing in June 2026, with 1,500 AI credits included per month and additional consumption billed after that. **Pro+** ($39/month) and **Max** ($100/month) exist for higher credit allowances, but **new sign-ups for all paid plans have been paused since April 2026**. If you don't already have a subscription, you can't get one.
...
uv venv ~/ws/vllm/.gguf-env
source ~/ws/vllm/.gguf-env/bin/activate
uv pip install transformers torch sentencepiece protobuf

# Run the conversion
cd ~/llama.cpp
python convert_hf_to_gguf.py ~/ws/vllm/Qwen2.5-Coder-1.5B \
  --outfile ~/ws/vllm/Qwen2.5-Coder-1.5B.gguf \
  --outtype f16
```
...
llama.cpp ships with a built-in HTTP server (`llama-server`) that exposes OpenAI-compatible endpoints. I started it with:

```bash
/home/dipankardas/llama.cpp/build/bin/llama-server \
  -m ~/ws/vllm/Qwen2.5-Coder-1.5B.gguf \
  --port 6969 \
  --host 127.0.0.1 \
  -c 8192 \
  -ngl all \
...
- Real-time responsiveness compared to cloud Copilot — 49 tok/s is usable but not instant
...
- **Context splitting with parallel slots**: `--parallel 4` with `-c 8192` gives each slot 2,048 tokens. For inline completion this is sufficient, but if you increase the context, adjust the slot count or total context accordingly.
...
- **No speculative decoding**: llama.cpp supports draft models for speculative decoding, but I didn't configure it. A 0.5B draft model could potentially increase generation throughput by 1.5-2x.
