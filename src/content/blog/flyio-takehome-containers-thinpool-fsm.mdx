...
| `target_type` | `thin-pool` | Kernel target module. Run `dmsetup targets` to list yours. Others in mainline include `linear`, `thin`, `striped`, `crypt`, `raid`, `snapshot`. |
...
2. **Density.** One physical host can carry thousands of thin volumes derived from a few base images. Shared blocks stay shared until something writes. This is the only reason a single fleet host can run thousands of customer VMs without burning through its NVMe in a week.
...
## From S3 to Snapshot: The Code Path

Now scale that flow. One image, by hand, in a tutorial, takes ten minutes. A real host has thousands of images, churns daily, crashes mid-extract, and cannot afford to redo work. That is what the Go binary handles.

It auto-discovers `*.tar.gz` keys under `images/` in the configured S3 bucket and feeds each one to the FSM. For each image, six transitions run:
...
`unpack-image` does the same `dmsetup`/`mkfs.ext4`/`tar -x` you ran by hand — wrapped in a function that reports back to the FSM:

```go
devicePath, _ := deps.DeviceMapper.CreateThinVolume(req.Msg.ImageID, 1024)
deps.DeviceMapper.FormatVolume(devicePath)
mountPoint, _ := deps.DeviceMapper.MountVolume(devicePath, req.Msg.ImageID)
deps.ImageUnpacker.ExtractImage(imagePath, mountPoint)
deps.DeviceMapper.UnmountVolume(mountPoint)
```

`create-snapshot` runs the two `dmsetup` calls from §6 — `create_snap` then `dmsetup create`. That is the "activation": the same primitives, automated, idempotent, restartable.
...
If you want the binary, not the tutorial: the repo ships Terraform for an EC2 box plus `setup-thinpool.sh`. End to end:

```bash
cd deploy/terraform && terraform init && terraform apply
ssh ubuntu@<IP>  # then clone the repo, build with CGO_ENABLED=1, run as root
```

A 4 GB thinpool comfortably holds half a dozen real images (alpine, python:alpine, golang:alpine, ubuntu:24.04, two ksctl images) with snapshots. Real fleet hosts run the same primitives on real NVMe. Same `dmsetup` calls. More zeros.
