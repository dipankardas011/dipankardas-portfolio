...
cover: "/img/blogs/k8s-sched-framework.webp"
...
![kubmin workload analysis showing Node Instance Recommendation with fitScore, cost savings, and alternative instance types](/img/blogs/kubmin-workload-instance-type-recomend.webp)
...
![Kubernetes scheduler framework — the filtering and scoring pipeline a pod goes through before being assigned to a node](/img/blogs/k8s-sched-framework.webp)
...
![kubectl proxy configz output showing kubeReserved, systemReserved, and evictionHard on kubeadm control plane and worker nodes](/img/blogs/k8s-avail-res-kubeadm-example.webp)

And the kubmin dashboard confirms the cluster is running with kubeadm v1.35 on AWS:

![kubmin dashboard showing a self-managed kubeadm v1.35 cluster running on AWS](/img/blogs/k8s-avail-res-kubeadm-example-kubmin.webp)
...
![kubectl proxy configz output showing kubelet reservations applied per-node on a K3s cluster](/img/blogs/k8s-avail-res-k3s-example.webp)

The kubmin dashboard for the K3s cluster:

![kubmin dashboard showing a self-managed K3s v1.35 cluster running on AWS](/img/blogs/k8s-avail-res-k3s-example-kubmin.webp)
