...
<Callout type="warning">
  One caveat before you read further: don't self-host a database because this post made it look easy. I went this route because constraints I couldn't move around forced my hand — cost ceilings, data residency, a workload the managed offerings in my region didn't fit cleanly. If Azure Database for MariaDB / MySQL, RDS, Cloud SQL, PlanetScale, or any managed equivalent works for your shape, take it. Backups, patching, failover, and the 3 AM pager all stop being yours. The story below is what to do when "use the managed one" isn't an option, not an argument that you should opt out of one.
</Callout>
...
**Docker waits for the mount, or it will silently destroy your day.** This one bit me on a reboot once and it's the reason I'm including it. The data disk is mounted via `/etc/fstab` (UUID-based, `defaults,nofail`), but `nofail` means "don't block boot if the mount fails" — *Docker still boots*. If the systemd `*.mount` unit hasn't finished by the time `docker.service` starts, MariaDB comes up on an empty `/mariadbdata` and cheerfully initializes a fresh, blank database directory on top of nothing. The container goes healthy. The application connects. You serve queries against an empty schema. The real data is sitting one mount away, fine, but unreachable.

The fix is a one-line ordering constraint in a `docker.service.d` drop-in:

```ini
[Unit]
Requires=mariadbdata.mount
After=mariadbdata.mount
```

`Requires=` says Docker should fail (not boot the container) if the mount didn't come up. `After=` says wait for it before starting. Together they turn "silently serving an empty DB" into "Docker doesn't start" — which is exactly the failure mode you want. The setup script writes this drop-in automatically as part of `mount`. The only reason to know about it is to recognize the symptom and not blame the database when you ever skip it.
...
<figure>
  <img src="/img/blogs/mariadb-pitr-timeline.svg" alt="PITR timeline: snapshot interval plus binlog replay to a target instant" />
  <figcaption>Two snapshots and a target — snap_T0 is the state base, snap_T1 is the binlog donor, replay carries you to T_target.</figcaption>
</figure>
...
Two backup paths flow off the same primary, and three recovery paths flow back in. Worth seeing them side by side.

<figure>
  <img src="/img/blogs/mariadb-backup-architecture.svg" alt="End-state architecture: snapshot path as primary, mariadb-backup disk as failsafe" />
  <figcaption>Two backup paths off the primary, three recovery doors back in — Vault snaps for fast/PITR, the backup disk as an independent failsafe.</figcaption>
</figure>

And the three DR doors:

| | **DR #1 — snap restore (non-PITR)** | **DR #2 — snap + binlog replay (PITR)** | **DR #3 — mariadb-backup restore** |
|---|---|---|---|
| **What you do** | Pick a snap. Restore disk → attach LUN 9 → boot → re-link replicas. | Pick `snap_T0 ≤ T_target` and `snap_T1 > T_target`. Boot T0 read-only. Mount T1 on scratch, harvest binlogs. Replay up to `T_target`. Promote. Rebuild replicas. | Restore backup disk → attach at LUN 9 (it *is* the datadir). `RESET MASTER`, set GTID from `mariadb_backup_binlog_info`. Boot. Re-link replicas. |
| **RPO** | ≤ 1 hour | 1 transaction (within last 7 days) | ≤ 24 hours |
| **RTO** | ~15 min | 30–60 min | ~15 min |
| **Granularity** | Snapshot point | Arbitrary timestamp | Backup point |
| **When to reach for it** | Hardware loss, plain rollback to a known-good hour. | Bad migration at 14:23 — land at 14:22:58. | Vault is gone, compromised, or recovery points are corrupt. |

Three doors, one hallway. Every one ends in the same thing — a managed disk attached at LUN 9 and a `START REPLICA` somewhere — because that's the only operation the database actually understands. Backup *strategy* differs; backup *primitive* doesn't.
