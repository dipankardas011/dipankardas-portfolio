...
So I split them. OS on its own disk. A dedicated Premium SSD attached at LUN 8, mounted at `/mariadbdata`, bind-mounted into the container as `/var/lib/mysql`.
...
Restore from this path is the same procedure as Stage 5: pull a disk from the vault, attach it at LUN 8, boot. There's nothing left to copy. It's a defense-in-depth path that's independent of the primary snapshot chain — different disk, different backup instance, different recovery point — but uses the exact same restore mechanism. Two parallel paths, one workflow.
